...
export function CursorGrid({
  cellSize = 60,
  color = '#a3a3a3', // Subtle neutral gray
  radius = 160,
  falloff = 'smooth',
  holdTime = 300,
  fadeDuration = 700,
  lineWidth = 1.0,
  maxOpacity = 0.4, // Light up opacity
  fillOpacity = 0.05,
  gridOpacity = 0.06, // More visible base grid
  cellRadius = 0,
  clickPulse = true,
  pulseSpeed = 600,
  className = ''
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: MouseEvent) => {
      ...
      const rect = canvas.getBoundingClientRect();
      // Allow cursor to trigger grid cells slightly outside canvas to feel smooth
      if (x >= -p.radius && x <= rect.width + p.radius && y >= -p.radius && y <= rect.height + p.radius) {
        energize(x, y);
        wake();
      }
    };
